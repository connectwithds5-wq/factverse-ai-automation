...
GEMINI_ENABLED = os.environ.get("GEMINI_STORYBOARD_ENABLED", "1") == "1"
...
def fallback_storyboard(data, reason=""):
    ...
    shots = [
        {"start": 0, "end": 4.5, "narration": concise(hook, 9), "caption": "The surprising fact", "visual_prompt": f"Open on a premium cinematic documentary visualization of this exact idea: {hook}. Show the real subject clearly in the first second, then make the key phenomenon visually obvious. Photorealistic, scientifically believable, dramatic natural lighting, slow controlled camera movement, vertical 9:16, no text, subtitles, logos or watermark."},
        {"start": 4.5, "end": 9.0, "narration": concise(fact, 9), "caption": "How it happens", "visual_prompt": f"Create a concrete cinematic visual explanation of this exact scientific fact: {fact}. Show the actual physical objects, environment, scale difference, motion or process described, rather than an unrelated person or generic science imagery. Premium photorealistic documentary style, clear subject, smooth camera movement, vertical 9:16, no written text, subtitles, logos or watermark."},
        {"start": 9.0, "end": 13.5, "narration": concise(fact, 9), "caption": "The key mechanism", "visual_prompt": f"Visually demonstrate the mechanism behind this statement: {fact}. Use a close, easy-to-understand physical demonstration or cinematic scientific visualization that directly corresponds to the words being spoken. Keep the main subject centered and readable, premium photorealistic documentary cinematography, vertical 9:16, controlled motion, no text, subtitles, logos or watermark."},
        {"start": 13.5, "end": 18.0, "narration": concise(twist, 9), "caption": "The surprising result", "visual_prompt": f"Create the final memorable documentary reveal for this exact conclusion: {twist}. Show the consequence or result literally and clearly, with a strong visual payoff rather than a generic abstract image. Premium photorealistic cinematic science documentary, subtle atmospheric motion, slow forward camera push, vertical 9:16, no text, subtitles, logos or watermark."},
    ]
...
def validate_board(board):
    ...
    for i, shot in enumerate(shots):
        ...
        if word_count > max_words:
            raise ValueError(f"Shot {i + 1} narration is too dense: {word_count} words for {duration:.1f}s (max {max_words})")
        previous_end = end
...
def request_gemini(prompt):
    ...
    for attempt in range(1, 4):
        try:
            response = requests.post(API.format(model=MODEL), params={"key": KEY}, json=body, timeout=90)
...
There is NO hard 15-second limit. Target 18-22 seconds, preferably 20-22 seconds, so the explanation is clear and natural.
...
Narration should explain the visual, not merely repeat the original fact. Visual prompts must not contain written text, subtitles, logos, watermarks, UI, or infographic labels.
...
{{"shots":[{{"start":0,"end":5,"narration":"...","caption":"...","visual_prompt":"..."}}]}}'''

    board = None
    if not GEMINI_ENABLED:
        print(f"INFO: Gemini storyboard generation disabled by zero-cost preflight for model '{MODEL}'.")
        board = fallback_storyboard(data, f"Configured Gemini model '{MODEL}' was not available during preflight")
    else:
        try:
            ...
            print("Using deterministic aligned fallback storyboard; continuing to Wan 2.2.")
            board = fallback_storyboard(data, str(exc))

    duration = validate_board(board)
    print(f"Storyboard validated: {len(board['shots'])} shots, {duration:.2f}s")
